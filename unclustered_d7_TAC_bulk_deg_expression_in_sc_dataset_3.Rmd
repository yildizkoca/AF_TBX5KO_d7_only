...
source("~/Desktop/projects/deji/scripts/functions.R")

#get DEGs from previous bulk analysis
bulk<-read.table("~/Desktop/projects/pott/bulk_rnaseq_data/Table_TAC_D9_resultsDESeq2.csv", sep=",", header=T)
rownames(bulk)<-bulk$gene_id
bulk<-bulk[,c("gene_id","log2FoldChange","padj")]
bulk<-na.omit(bulk)
sig_bulk<-bulk[as.numeric(bulk[,"padj"]) < 0.05,]
#sig_bulk<-sig_bulk[abs(as.numeric(sig_bulk[,"log2FoldChange"])) > 0.5, ]
sig_bulk_down<-sig_bulk[as.numeric(sig_bulk[,"log2FoldChange"]) < 0,]
sig_bulk_down<-orderby(sig_bulk_down, columns="padj")[1:2000,]
sig_bulk_up<-sig_bulk[as.numeric(sig_bulk[,"log2FoldChange"]) > 0,]
sig_bulk_up<-orderby(sig_bulk_up, columns="padj")[1:2000,]
...
split_column1 <- factor(split_column1, levels = order)
split_column2 <- factor(split_column2, levels = order2)
...
Heatmap(t(scale(t(nc_sig_bulk_up))), top_annotation = HeatmapAnnotation(foo=split_column1, foo2=split_column2, col = list(foo=col_column1, foo2=col_column2, at=order), show_annotation_name = FALSE, annotation_legend_param = list(foo=list(title="", at=order),foo2=list(title="", at=order2))), show_column_names = FALSE, show_row_names = FALSE,  heatmap_legend_param = list(title="log2(cpm+1)"), cluster_columns = F)
dev.off()
...
d7_dge<-d7_dge[,c("logFC","adj.P.Val")]
d7_dge<-na.omit(d7_dge)
...
ggsave("d7_dream_pseudobulk_vs_TAC_bulk_DGE_labeled_colored.pdf", plot=g, width = 8, height = 8)
...
g_up<-ggVennDiagram(list(psuedobulk_DEGs=rownames(sig_pseudo_d7_up),  bulk_DEGs=rownames(sig_bulk_up))) + scale_fill_distiller(palette = "Reds",   direction = 1) + coord_flip() + theme(
    legend.position = "right",
    plot.margin = margin(1, 1, 1, 1, "cm")  # Add extra space for the legend
  )
#ggsave("d7_dream_pseudobulk_vs_bulk_sig_up_DGE_Venn.pdf", plot=g_up, width = 8, height = 6)

g_down<-ggVennDiagram(list(psuedobulk_DEGs=rownames(sig_pseudo_d7_up),  bulk_DEGs=rownames(sig_bulk_up))) + scale_fill_distiller(palette = "Reds",   direction = 1) + coord_flip() + theme(
    legend.position = "right",
    plot.margin = margin(1, 1, 1, 1, "cm")  # Add extra space for the legend
  )
#ggsave("d7_dream_pseudobulk_vs_bulk_sig_down_DGE_Venn.pdf", plot=g_up, width = 8, height = 6)
